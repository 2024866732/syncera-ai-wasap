...
- **Async Python (FastAPI):** sync file I/O (`json.load`/`json.dump`/`open`), sync ORM queries, `subprocess.run()` inside `async def` block the event loop. Should use `run_in_executor`, `asyncio.create_subprocess_exec`, or async drivers.
- **Webhook handlers:** signature verification must not silently succeed when secrets are unset
