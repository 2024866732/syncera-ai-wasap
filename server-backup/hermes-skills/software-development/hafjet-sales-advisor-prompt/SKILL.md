...
- **Sarah guardrail integration (Jul 2026):** Sarah prompt is now a separate constant (`SARAH_SYSTEM_PROMPT` in `sarah_prompt.py`) imported into `hermes_ai.py`. Guardrails (`is_human_takeover_request`, `is_out_of_scope`) run at the top of `ask_hermes()` BEFORE any LLM call — this ensures instant rejection/response without LLM latency or cost. Do NOT move guardrails inside `ask_openrouter()` or `ask_hermes_cli()`; they must intercept first.
- **Separate prompt files:** `sarah_prompt.py` holds the full Sarah system prompt + static KB (pricing, ansuran plans, catalog link). `hermes_ai.py` imports `SARAH_SYSTEM_PROMPT` and sets `SOUL_CONTEXT = SARAH_SYSTEM_PROMPT` directly (no module-level f-string concat). This keeps the prompt as a single source of truth and avoids stale `SOUL_CONTEXT` at import time.
- **Static KB not generated by LLM:** Repair pricing (iPhone 11 LCD AA RM120 / ORI RM180), RedONE plan details, catalog link are hardcoded in `sarah_prompt.py` dicts (`REPAIR_PRICING`, `ANSURAN_PLANS`). The LLM reads them from the system prompt — do NOT let the model hallucinate prices.
- **Human takeover DB flag TODO:** Current implementation returns the takeover message but does NOT yet set a DB flag to stop auto-reply for that conversation. When implementing, add a `conversation_mode` or `human_takeover_until` column to `customers`/`conversations` table and check it at the top of `ask_hermes()` or in `webhook_listener.generate_reply()`.
